	...
	<view>
		...
		<view class="goods-section">
			<view class="g-header b-b">
				<image class="logo" src="../../static/nuoMi.png"></image>
				<text class="name">糯米酒吧</text>
			</view>
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item,index) in dataList" :key="index" v-if="dataList.length>0">
				<image :src="filePath+item.nmGoods.pic"></image>
				<view class="right">
					<text class="title clamp">{{item.nmGoods.title}}</text>
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell desc-cell" style="display: flex;align-items: flex-start;">
				<text class="cell-tit clamp">备注</text>
				
				<textarea class="desc" style="padding-top: 20rpx;" type="text" v-model="memo" placeholder="请填写备注信息,包括您的座位/商品要求等."  />
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			submit(){
				...
				let obj = {
					priceSum : this.totalPrice,
					state:0,
					memo:this.memo,
					nmBarCartList:this.dataList
				}
				
				let _this = this;
				appRequest.request({
				  data: obj,
				  url: appRequest.urlMap.createNmOrder,
				  success: function(res) {
				    if (res.data.code == 200) {
				      appRequest.msg("订单提交成功");
					  uni.redirectTo({
					  	url: '/pages/money/pay?id='+ res.data.data.id+"&price="+_this.totalPrice
					  })
				    } else {
